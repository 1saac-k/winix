...
AirPurifierDevice_TypeName = "custom_components.winix.driver.AirPurifierDevice"
...
async def test_wrapper_update(
    ...
    with patch(
        f"{AirPurifierDevice_TypeName}.get_state",
...
async def test_async_ensure_on() -> None:
    ...
    with patch(f"{AirPurifierDevice_TypeName}.turn_on") as turn_on:
        wrapper = build_mock_wrapper()
...
async def test_async_turn_off() -> None:
    """Test turning off."""
    with (
        patch(f"{AirPurifierDevice_TypeName}.turn_on") as turn_on,
        patch(f"{AirPurifierDevice_TypeName}.turn_off") as turn_off,
    ):
        wrapper = build_mock_wrapper()
        assert not wrapper.is_on  # initially off
...
async def test_async_auto() -> None:
    ...
    with patch(f"{AirPurifierDevice_TypeName}.auto") as auto:
        wrapper = build_mock_wrapper()
...
async def test_async_plasmawave_on_off() -> None:
    ...
    with (
        patch(f"{AirPurifierDevice_TypeName}.plasmawave_on") as plasmawave_on,
        patch(f"{AirPurifierDevice_TypeName}.plasmawave_off") as plasmawave_off,
    ):
        wrapper = build_mock_wrapper()
...
async def test_async_manual() -> None:
    ...
    with patch(f"{AirPurifierDevice_TypeName}.manual") as manual:
        wrapper = build_mock_wrapper()
...
async def test_async_sleep() -> None:
    ...
    with patch(f"{AirPurifierDevice_TypeName}.sleep") as sleep:
        wrapper = build_mock_wrapper()
...
async def test_async_set_speed() -> None:
    ...
    with (
        patch(f"{AirPurifierDevice_TypeName}.turn_on"),
        patch(f"{AirPurifierDevice_TypeName}.manual"),
        patch(f"{AirPurifierDevice_TypeName}.high") as high_speed,
        patch(f"{AirPurifierDevice_TypeName}.low") as low_speed,
    ):
        wrapper = build_mock_wrapper()
